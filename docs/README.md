...
- simulation preset (optional, single-run)
- particle mode
- backend
- mesh adapter
- duration
...
- `Simulation Preset` applies a ready-made parameter profile for one run.
- `Batch Execution Mode` is still a separate multi-run workflow.
...
It also shows a historical runtime hint (median/p75) after you collect a few runs with similar parameters.
...
The UI also prints run start time, end time, and elapsed duration after completion.
...
You can build runs from the form-based mini-builder.
Optional: import or export JSON when you want to share or version batch plans.

Each run object may override only the values you want to change.
...
Example JSON (for import/export):
