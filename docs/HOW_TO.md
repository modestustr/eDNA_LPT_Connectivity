...
## Use Simulation Presets (Single Run)

Use `Simulation Preset` for quick one-run setup.

Recommended interpretation:

- `Quick Smoke`: fast sanity run
- `Balanced Analysis`: general-purpose analysis run
- `Station Focus`: station-oriented configuration
- `Long Drift (Heavy)`: heavier repeated-release scenario

Important distinction:

- Preset = one run profile
- Batch = multiple run scenarios
...
When enough similar runs exist, this panel also shows a historical runtime estimate (median/p75) for the current parameter profile.

## Run the Simulation

Click `Run Simulation`.

After completion, the app reports start time, end time, and elapsed duration.
...
Use the form-based mini-builder to add/edit run rows.
You can also import/export JSON when needed.
...
Tip: if some batch items use mesh-formatted files, set `mesh_adapter` per row (or in imported JSON).
